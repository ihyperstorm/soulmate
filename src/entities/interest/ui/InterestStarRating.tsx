...
export function InterestStarRating({ value, onChangeAction, size = 24 }: Props) {
	const t = useTranslations("rating")
	const clamped = Math.min(5, Math.max(0, Math.floor(value)))

	return (
		<div className="flex gap-0.5 items-center justify-center" role="group" aria-label={t("groupLabel")}>
			{[1, 2, 3, 4, 5].map((n) => (
				<button
					key={n}
					type="button"
