...
| Фаза | Что | Статус |
|---|---|---|
| 0 | Данные: `moods` / `moodUpdatedAt` в `User` | ✅ сделано |
| 1 | Сущность `entities/mood` (каталог, TTL, матч, `MoodChip`) | ✅ сделано |
| 2 | API `PATCH` / `DELETE /api/users/me/mood` | ✅ сделано |
| 3 | Фича `features/mood/set-mood` — `useSetMood` + `MoodPicker` | ✅ сделано |
| 4 | Виджет `MoodBoard` на дашборде | ✅ сделано |
| 5 | Mood в матчинге | ✅ сделано |
| 6 | Conversation starters по настроению | ✅ сделано |
| 7 | i18n | ✅ сделано |

**Фича готова целиком.** Настроение задаётся с дашборда и из профиля, участвует
в сортировке всех списков людей, видно в карточке и меняет первый айсбрейкер.

> ⚠️ **Чтобы это увидеть, настроение нужно не только себе.** Буст требует
> совпадения с обеих сторон: `sharedMoods(мои, чужие)`. Пока настрой задал один
> человек, `moodBoost` тождественно нулевой и порядок карточек не меняется —
> это cold start из решения №4, а не поломка. Для проверки нужно проставить
> настроения хотя бы паре тестовых аккаунтов.

**Известное ограничение `DailyMatch`.** `pickDaily` не зависит от порядка внутри
пула — он детерминированно выбирает из топ-5. Буст меняет порядок, но выбор
изменится, только если сдвинется **состав** пятёрки (максимум буста — 16 очков).
То есть настроение заметнее работает в списках, чем в блоке человека дня.
...
**Срок настраивается:** `NEXT_PUBLIC_MOOD_TTL_HOURS`, дефолт 24. Локально ставится
длинным (`720`), иначе засеянные тестовые настроения протухают посреди отладки —
на это уже напоролись. Префикс `NEXT_PUBLIC_` обязателен, потому что TTL считается
в браузере; с обычной переменной значение на клиенте было бы `undefined`.

**В базе ничего не чистится.** Протухшее настроение остаётся лежать в `moods`,
`getActiveMoods` просто перестаёт его отдавать. Это значит, что в Mongo глазами
видны значения, которых приложение уже не учитывает — выглядит странно, но
альтернативы хуже: TTL-индекс Mongo удаляет документ целиком (то есть снёс бы
пользователя), а ленивая чистка означала бы запись на read-пути. Вопрос закроется
сам, когда ранжирование поедет на сервер вместе с пагинацией `/api/users`:
в запрос добавится `moodUpdatedAt: {$gt: cutoff}`, и сырые значения перестанут
уходить на клиент.
...
### ✅ Фаза 3 — фича `features/mood/set-mood` · 💰 ~1.5 ч · **сделано**
...
### ✅ Фаза 4 — виджет на дашборде · 💸 ~40 мин · **сделано**
...
### ✅ Фаза 5 — mood в матчинге · 💰 ~1.5 ч · **сделано**
...
### ✅ Фаза 6 — conversation starters по настроению · 💰 ~1 ч · **сделано**
...
`pickQuestion` остаётся как есть — его единственный вызов в `UserCard.tsx:87`.
...
остальные — из общих интересов. Та же `useMemo`-обёртка по ключу, что и сейчас
(внутри `pickQuestion` живёт `Math.random`, см. комментарий на строках 78–80).

> Это тот момент, где фича перестаёт быть декоративной: общий настрой напрямую меняет
> первую фразу, которую человек отправит.

Единственная точка выхода вопросов — карточка конкретного человека. Виджет
`TalkIdeas` на дашборде удалён 03.09.2026: обезличенные «темы для разговора»
на основе только своих интересов не отвечали на вопрос «что написать **этому**
человеку», а блок «Растопить лёд» в `UserCard` отвечает. Отдельного блока
starters на дашборде в этой фазе не появляется — если понадобится, он вырастет
из `MoodBoard` (фаза 4), где у вопроса уже есть контекст выбранного настроения.

---

### ✅ Фаза 7 — i18n · 💸 ~40 мин · **сделано**
