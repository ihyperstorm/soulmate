...
|---|---|---|
| Фидбэк и находки | [FEEDBACK.md](FEEDBACK.md) | 🟡 15 / 17 — открыт только OAuth |
...
> ⚠️ `TIPTOPPAY_PUBLIC_ID` и `TIPTOPPAY_API_SECRET` в текущем `.env` отсутствуют.
> Без них работает всё, кроме серверных вызовов платежей (`/api/payments/charge`
> и проверка подписи вебхука) — они бросят исключение при первом обращении.
...
**Восстановление пароля.** `/forgot-password` выпускает одноразовый токен
(SHA-256 в базе, час жизни) и шлёт ссылку письмом. `/reset-password` его гасит
атомарно, ставит пароль и отзывает все сессии. Эндпоинт запроса всегда отвечает
200, чтобы по коду ответа нельзя было перебрать зарегистрированные адреса.

**Смена учётных данных.** Пароль — `PATCH /api/auth/password`, почта —
`PATCH /api/auth/email`, обе в `/settings`. Каждая требует текущий пароль:
иначе угнанная сессия позволяет переписать адрес на свой и увести аккаунт
насовсем. Смена почты уведомляет старый адрес.

### Письма

`shared/lib/mailer` — одна функция `sendMail()`. Без `RESEND_API_KEY` письмо
печатается в консоль: этого достаточно, чтобы разрабатывать флоу целиком,
ссылку копируешь из терминала. С ключом уходит через Resend обычным REST,
npm-зависимости нет.

Ссылки в письмах строятся из `APP_URL`, а не из заголовка `Host` запроса —
иначе подменой `Host` жертве уходит ссылка сброса на чужой домен.
