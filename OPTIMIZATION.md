...
### 🟢 12. i18n — пока копится debt
- **Где:** Микс EN/RU в UI (часть текстов RU в комментариях кода, UI на EN).
- **Почему важно:** Если будет локализация — переделывать.
- **Фикс:** `next-intl` или `next-i18next`. Сейчас не критично.
- **💎 Expensive** — отложить, пока не нужно.
