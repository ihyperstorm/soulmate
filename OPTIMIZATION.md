...
Самое дешёвое из оставшегося — **пункт 6 производительности** (N+1 в `/api/chats`,
одна aggregation вместо цикла) и **пункт 9 безопасности** (убрать `userId`
из `localStorage` и из тела запроса `/api/interests`).
...
- **Фикс:** Email confirmation flow с одноразовой ссылкой. Почтовый канал и хранилище одноразовых токенов общие с восстановлением пароля — делать одной задачей.
