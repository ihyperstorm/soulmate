...
Единственный оставшийся 🔴 — **пункт 2**: `PATCH /api/users/[id]` не проверяет
владельца, любой залогиненный может править чужой профиль. Фикс — три строки.
...
### 🟡 15. Нет UI для «выйти со всех устройств»
- **Где:** `src/widgets/sidebar/ui/AppSidebar.tsx:98`, `src/features/auth/logout/ui/LogoutButton.tsx:24`
- **Что:** Бэкенд умеет отзывать все сессии — `POST /api/auth/logout` с телом `{all: true}` вызывает `revokeAllSessions`. Но оба места в интерфейсе шлют запрос **без тела**, то есть всегда обычный logout с одного устройства. Дёрнуть отзыв всех сессий из приложения нечем.
- **Почему важно:** Для обычной кнопки «Выйти» текущее поведение правильное — вышел на ноутбуке, телефон остался залогиненным. Но при компрометации («забыл выйти на чужом компьютере», «угнали аккаунт») у пользователя **нет ни одного способа** убить чужую сессию: refresh живёт 30 дней, а смены пароля в проекте тоже нет (см. ниже). Возможность есть, доступа к ней нет.
- **Фикс:** Пункт «Выйти на всех устройствах» → `axios.post('/api/auth/logout', {all: true})` + два ключа в `messages/{ru,en}.json`. Класть лучше в `/settings` рядом с премиумом, а не в дропдаун сайдбара: по смыслу это настройка безопасности, и случайно нажать сложнее.
- **💸 Cheap** — пункт меню + i18n, бэкенд готов.

### 🟡 16. Нет смены пароля
- **Где:** `src/app/api/auth/*` — эндпоинта нет
- **Что:** Пароль задаётся при регистрации и больше не меняется.
- **Почему важно:** Базовая функция аккаунта. Плюс это второй потребитель `revokeAllSessions`: при смене пароля все сессии должны отзываться, иначе смена пароля не выкидывает того, кто уже внутри.
- **Фикс:** `PATCH /api/auth/password` — проверить текущий пароль через bcrypt, записать новый хэш, вызвать `revokeAllSessions(userId)`, выдать новую сессию текущему устройству через `issueSession`. Плюс форма в `/settings`.
- **💰 Medium** — эндпоинт + форма + валидация.
